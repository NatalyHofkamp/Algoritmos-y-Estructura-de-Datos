...
bool list_iter_insert_after(list_iter_t *iter, void *value){
    if(list_is_empty(iter->list) || list_iter_at_last(iter) ){
        bool insert = list_insert_tail(iter->list,value);   
...
bool list_iter_insert_before(list_iter_t *iter, void *value){
    bool insert;
    if(list_iter_at_first(iter) && !list_is_empty(iter->list)){
        insert= list_insert_head(iter->list,value);
        list_iter_backward(iter);
        if (!insert) return NULL;
    }else{
        node_t* aux = iter->curr;
        list_iter_backward(iter);
        insert = list_iter_insert_after(iter,value);
        iter->curr = aux;
        if(!insert) return NULL;
    }
    return true;
...
void *list_iter_delete(list_iter_t *iter){
     ...
     node_t* new_node = iter->curr->next;
